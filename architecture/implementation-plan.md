...
with its findings. Next: Phase 1.
...
Deliverables:
- Solution, `Directory.Build.props` (nullable, warnings as errors, analyzers), `Directory.Packages.props` (central versions).
- `OrderPlatform.ServiceDefaults`: OpenTelemetry (traces, metrics, logs, OTLP exporter), health checks (`/health/live`, `/health/ready`), standard HTTP resilience.
...
- `BuildingBlocks`: `RelationalOutbox` helper enlisting an `NpgsqlTransaction` in the Wolverine outbox (ADR-0007).
...
- `deploy/docker-compose.yml`: Postgres, Keycloak, Migrator, Api, standalone Aspire Dashboard as OTLP receiver;
  `deploy/keycloak/orderplatform-realm.json` with two accounts, a portal user and a support agent.
...
- Feature flags: `IFeatureFlags` in `BuildingBlocks`, Microsoft.FeatureManagement adapter in the Api host (records
  evaluations on the current span), per-module `FeatureFlags` registry convention, flag configuration in appsettings
  with reload on change, and a test helper to run a test with a flag on and off (ADR-0019).
- Module shells for all **six** modules (Ordering, Customers, Pricing, Inventory, Billing, Shipping — four projects each, empty `IModule` registration).
- Test projects and shared test infrastructure per ADR-0022 (xUnit v3, Shouldly, Testcontainers fixture that runs the
  Migrator, WireMock.Net base class, flag on/off helper, test data builder convention).
...
- CI pipeline (`.github/workflows`) implementing the pull request pipeline and the image build stage of ADR-0021
  (incl. `codegen test` and the Marten patch artifact); deployment stages, including the rollback floor check, stubbed
  until a hosting platform is chosen.

Acceptance criteria:
- `docker compose up` and `dotnet run --project src/AppHost/...` both start the system, run migrations and show traces in the dashboard.
- A token obtained from local Keycloak calls an authenticated health/echo endpoint; a request without a token gets `401`.
...
- The Api container starts in static code generation mode and passes the smoke test.
- No application project references `Microsoft.FeatureManagement` directly (architecture test); a missing flag evaluates to off.
- Starting the Api with `ASPNETCORE_ENVIRONMENT=Production` and a `Fake` integration fails at startup.
- The pull request pipeline runs green, including the compose smoke test.
...
| Phase 1 | Full, CI deployment stages stubbed | 2 h |
